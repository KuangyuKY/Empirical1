...
**在 VM 上跑 Stata 的两条注意**
- 整个文件跑：命令窗口输入 `do "…/pipeline/04_entry.do"`，或在编辑器里**不选中任何文字**按 Ctrl+D。只选一段运行会跳过文件头的 `cd`；把命令粘贴进命令窗口逐行执行时，出错也不会停
- `git pull` 挪动过文件后，关掉编辑器里还开着的旧缓冲再打开，不然跑的还是旧内容
- **do 文件的注释里绝对不要出现 `/*`**（比如写路径 `results/entry/*.txt`）。Stata 会把 `*` 注释行里的 `/*` 也当成块注释开头，没有 `*/` 闭合就一直注释到文件末尾——**整个文件一行都不执行、也不报错**，结果窗口里还会把源代码原样滚一遍，看起来像跑完了。2026-09-13 的 04 就是这样白跑了一次
...
展开 625,619,601 行。（它当年在 VM 上从没真正跑起来过：文件头注释里有 `/*`，整个文件被注释掉了；
另外在命令窗口里手动读全部 30 列 `full_data` 时确实撞过 r(909)。）现在 03 只在产品对层面构造 choice set（148,584 对），
