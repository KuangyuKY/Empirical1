...
*
* 输入（$DATA，只读，不修改）
*   buyer-yearly-17-collapsed.dta   已改好列名并带 year / input_output：
*   buyer-yearly-18-collapsed.dta     firm_id product_id v_positive v_negative
*   seller-yearly-17-collapsed.dta    year input_output
*   seller-yearly-18-collapsed.dta
*   bianma_all.dta                  product_id(19位) + 名称
*
* 输出（$OUT）
...
append using "$DATA/seller-yearly-17-collapsed.dta"
append using "$DATA/buyer-yearly-18-collapsed.dta"
append using "$DATA/seller-yearly-18-collapsed.dta"
...
*########## 4. 按 19 位 collapse（★ by() 保留 year）+ 去非正值 ##########
...
*########## 5. 截 15 位 -> lenth15_year（只留有产出的企业）##########
